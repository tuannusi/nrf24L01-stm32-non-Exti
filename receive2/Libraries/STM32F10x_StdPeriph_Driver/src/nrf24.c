...
static uint8_t nRF24_ReadReg(uint8_t reg) {
	...
	nRF24_CSN_L_2();
	nRF24_LL_RW_2(reg & nRF24_MASK_REG_MAP);
	value = nRF24_LL_RW_2(nRF24_CMD_NOP);
	nRF24_CSN_H_2();

	return value;
}
static uint8_t nRF24_ReadReg_1(uint8_t reg) {
	uint8_t value;

	nRF24_CSN_L_1();
	nRF24_LL_RW_1(reg & nRF24_MASK_REG_MAP);
	value = nRF24_LL_RW_1(nRF24_CMD_NOP);
	nRF24_CSN_H_1();
...
static void nRF24_WriteReg(uint8_t reg, uint8_t value) {
	...
	if (reg < nRF24_CMD_W_REGISTER) {
		...
		nRF24_LL_RW_2(nRF24_CMD_W_REGISTER | (reg & nRF24_MASK_REG_MAP));
		nRF24_LL_RW_2(value);
	} else {
...
static void nRF24_WriteReg_1(uint8_t reg, uint8_t value) {
	nRF24_CSN_L_1();
	if (reg < nRF24_CMD_W_REGISTER) {
		// This is a register access
		nRF24_LL_RW_1(nRF24_CMD_W_REGISTER | (reg & nRF24_MASK_REG_MAP));
		nRF24_LL_RW_1(value);
	} else {
		// This is a single byte command or future command/register
		nRF24_LL_RW_1(reg);
...
static void nRF24_ReadMBReg(uint8_t reg, uint8_t *pBuf, uint8_t count) {
	...
	nRF24_LL_RW_2(reg);
	while (count--) {
		*pBuf++ = nRF24_LL_RW_2(nRF24_CMD_NOP);
	}
	nRF24_CSN_H_2();
}

static void nRF24_ReadMBReg_1(uint8_t reg, uint8_t *pBuf, uint8_t count) {
	nRF24_CSN_L_1();
	nRF24_LL_RW_1(reg);
	while (count--) {
		*pBuf++ = nRF24_LL_RW_1(nRF24_CMD_NOP);
	}
	nRF24_CSN_H_1();
}
...
static void nRF24_WriteMBReg_1(uint8_t reg, uint8_t *pBuf, uint8_t count) {
	nRF24_CSN_L_1();
	nRF24_LL_RW_1(reg);
	while (count--) {
		nRF24_LL_RW_1(*pBuf++);
...
uint8_t nRF24_Check_1(void) {
	uint8_t rxbuf[5];
	uint8_t i;
	uint8_t *ptr = (uint8_t *)nRF24_TEST_ADDR;

	// Write test TX address and read TX_ADDR register
	nRF24_WriteMBReg_1(nRF24_CMD_W_REGISTER | nRF24_REG_TX_ADDR, ptr, 5);
	nRF24_ReadMBReg_1(nRF24_CMD_R_REGISTER | nRF24_REG_TX_ADDR, rxbuf, 5);
...
void nRF24_SetPowerMode_1(uint8_t mode) {
	uint8_t reg;

	reg = nRF24_ReadReg_1(nRF24_REG_CONFIG);
	if (mode == nRF24_PWR_UP) {
		// Set the PWR_UP bit of CONFIG register to wake the transceiver
		// It goes into Stanby-I mode with consumption about 26uA
		reg |= nRF24_CONFIG_PWR_UP;
	...
	nRF24_WriteReg_1(nRF24_REG_CONFIG, reg);
}
...
void nRF24_SetOperationalMode_1(uint8_t mode) {
	uint8_t reg;
...
void nRF24_SetCRCScheme_1(uint8_t scheme) {
	uint8_t reg;
...
void nRF24_SetRFChannel_1(uint8_t channel) {
	nRF24_WriteReg_1(nRF24_REG_RF_CH, channel);
}
...
void nRF24_SetAutoRetr_1(uint8_t ard, uint8_t arc) {
	// Set auto retransmit settings (SETUP_RETR register)
	nRF24_WriteReg_1(nRF24_REG_SETUP_RETR, (uint8_t)((ard << 4) | (arc & nRF24_MASK_RETR_ARC)));
}
...
void nRF24_SetAddrWidth_1(uint8_t addr_width) {
	nRF24_WriteReg_1(nRF24_REG_SETUP_AW, addr_width - 2);
}
...
void nRF24_SetAddr(uint8_t pipe, const uint8_t *addr) {
	...
	switch (pipe) {
		...
		case nRF24_PIPE1:
			...
			addr += addr_width;
			nRF24_CSN_L_2();
			nRF24_LL_RW_2(nRF24_CMD_W_REGISTER | nRF24_ADDR_REGS[pipe]);
			do {
				nRF24_LL_RW_2(*addr--);
			} while (addr_width--);
			nRF24_CSN_H_2();
...
void nRF24_SetAddr_1(uint8_t pipe, const uint8_t *addr) {
	uint8_t addr_width;
	...
	switch (pipe) {
		...
		case nRF24_PIPE1:
			// Get address width
			addr_width = nRF24_ReadReg_1(nRF24_REG_SETUP_AW) + 1;
			// Write address in reverse order (LSByte first)
			addr += addr_width;
			nRF24_CSN_L_1();
			nRF24_LL_RW_1(nRF24_CMD_W_REGISTER | nRF24_ADDR_REGS[pipe]);
			do {
				nRF24_LL_RW_1(*addr--);
			} while (addr_width--);
			nRF24_CSN_H_1();
		...
		case nRF24_PIPE5:
			// Write address LSBbyte (only first byte from the addr buffer)
			nRF24_WriteReg_1(nRF24_ADDR_REGS[pipe], *addr);
			break;
		default:
			// Incorrect pipe number -> do nothing
			break;
...
void nRF24_SetTXPower_1(uint8_t tx_pwr) {
	uint8_t reg;

	// Configure RF_PWR[2:1] bits of the RF_SETUP register
	reg  = nRF24_ReadReg_1(nRF24_REG_RF_SETUP);
	reg &= ~nRF24_MASK_RF_PWR;
	reg |= tx_pwr;
	nRF24_WriteReg_1(nRF24_REG_RF_SETUP, reg);
}
...
void nRF24_SetDataRate_1(uint8_t data_rate) {
	uint8_t reg;
...
void nRF24_SetRXPipe_1(uint8_t pipe, uint8_t aa_state, uint8_t payload_len) {
	uint8_t reg;

	// Enable the specified pipe (EN_RXADDR register)
	reg = (nRF24_ReadReg_1(nRF24_REG_EN_RXADDR) | (1 << pipe)) & nRF24_MASK_EN_RX;
	nRF24_WriteReg_1(nRF24_REG_EN_RXADDR, reg);

	// Set RX payload length (RX_PW_Px register)
	nRF24_WriteReg_1(nRF24_RX_PW_PIPE[pipe], payload_len & nRF24_MASK_RX_PW);

	// Set auto acknowledgment for a specified pipe (EN_AA register)
	reg = nRF24_ReadReg_1(nRF24_REG_EN_AA);
	if (aa_state == nRF24_AA_ON) {
		reg |=  (1 << pipe);
	} else {
		reg &= ~(1 << pipe);
	}
	nRF24_WriteReg_1(nRF24_REG_EN_AA, reg);
}
...
void nRF24_ClosePipe_1(uint8_t pipe) {
	uint8_t reg;

	reg  = nRF24_ReadReg_1(nRF24_REG_EN_RXADDR);
	reg &= ~(1 << pipe);
	reg &= nRF24_MASK_EN_RX;
	nRF24_WriteReg_1(nRF24_REG_EN_RXADDR, reg);
}
...
void nRF24_EnableAA_1(uint8_t pipe) {
	uint8_t reg;
...
void nRF24_DisableAA_1(uint8_t pipe) {
	uint8_t reg;

	if (pipe > 5) {
		// Disable Auto-ACK for ALL pipes
		nRF24_WriteReg_1(nRF24_REG_EN_AA, 0x00);
	} else {
		// Clear bit in the EN_AA register
		reg  = nRF24_ReadReg_1(nRF24_REG_EN_AA);
		reg &= ~(1 << pipe);
...
uint8_t nRF24_GetStatus_1(void) {
	return nRF24_ReadReg_1(nRF24_REG_STATUS);
}
...
uint8_t nRF24_GetIRQFlags_1(void) {
	return (nRF24_ReadReg_1(nRF24_REG_STATUS) & nRF24_MASK_STATUS_IRQ);
}
...
uint8_t nRF24_GetStatus_RXFIFO_1(void) {
	return (nRF24_ReadReg_1(nRF24_REG_FIFO_STATUS) & nRF24_MASK_RXFIFO);
}
...
uint8_t nRF24_GetStatus_TXFIFO_1(void) {
	return ((nRF24_ReadReg_1(nRF24_REG_FIFO_STATUS) & nRF24_MASK_TXFIFO) >> 4);
}
...
uint8_t nRF24_GetRXSource_1(void) {
	return ((nRF24_ReadReg_1(nRF24_REG_STATUS) & nRF24_MASK_RX_P_NO) >> 1);
}
...
uint8_t nRF24_GetRetransmitCounters_1(void) {
	return (nRF24_ReadReg_1(nRF24_REG_OBSERVE_TX));
}
...
void nRF24_ResetPLOS_1(void) {
	uint8_t reg;

	// The PLOS counter is reset after write to RF_CH register
	reg = nRF24_ReadReg_1(nRF24_REG_RF_CH);
	nRF24_WriteReg_1(nRF24_REG_RF_CH, reg);
}
...
void nRF24_FlushTX_1(void) {
	nRF24_WriteReg_1(nRF24_CMD_FLUSH_TX, nRF24_CMD_NOP);
}
...
void nRF24_FlushRX_1(void) {
	nRF24_WriteReg_1(nRF24_CMD_FLUSH_RX, nRF24_CMD_NOP);
}
...
void nRF24_ClearIRQFlags_1(void) {
	uint8_t reg;

	// Clear RX_DR, TX_DS and MAX_RT bits of the STATUS register
	reg  = nRF24_ReadReg_1(nRF24_REG_STATUS);
	reg |= nRF24_MASK_STATUS_IRQ;
	nRF24_WriteReg_1(nRF24_REG_STATUS, reg);
}
...
void nRF24_WritePayload_1(uint8_t *pBuf, uint8_t length) {
	nRF24_WriteMBReg_1(nRF24_CMD_W_TX_PAYLOAD, pBuf, length);
}
...
nRF24_RXResult nRF24_ReadPayload_1(uint8_t *pBuf, uint8_t *length) {
	uint8_t pipe;

	// Extract a payload pipe number from the STATUS register
	pipe = (nRF24_ReadReg_1(nRF24_REG_STATUS) & nRF24_MASK_RX_P_NO) >> 1;

	// RX FIFO empty?
	if (pipe < 6) {
		// Get payload length
		*length = nRF24_ReadReg_1(nRF24_RX_PW_PIPE[pipe]);

		// Read a payload from the RX FIFO
		if (*length) {
			nRF24_ReadMBReg_1(nRF24_CMD_R_RX_PAYLOAD, pBuf, *length);
		}
